...
void Laborator2::OnKeyPress(int key, int mods)
{
	...
	//La apasare 1 nr instante creste
	if (key == GLFW_KEY_1) {
		no_of_instances++;
	}
	//La apasare 2 nr instante scade
	if (key == GLFW_KEY_2) {
		no_of_instances--;
	}
	//La apasare 3 nr puncte generate creste
	if (key == GLFW_KEY_3) {
		no_of_generated_points++;
	}
	//La apasare 4 nr puncte generate scade
	if (key == GLFW_KEY_4) {
		no_of_generated_points--;
	}
	cout << "Instances: " << no_of_instances << " Generated points: " << no_of_generated_points << endl;
};
